...
- **기간제 인증**: AUTO 인증은 **1개월 / 6개월 / 1년** 버튼으로 기간을 정해 부여합니다
  (이미 AUTO 인 회원에게 누르면 **기존 만료일 기준으로 이어서 연장** — 남은 기간이 보존됩니다).
  만료되면 자동으로 Diary 로 돌아가고
  자동매매도 즉시 차단됩니다 — DB 시간별 검사(pg_cron, 마이그레이션 20260716g) +
  무인 러너 + 앱의 3중 검사로 동작합니다. 회원 헤더 배지에는 `AUTO D-27`처럼 남은 기간이 표시됩니다.
...
이름·이메일·휴대폰으로 회원을 검색할 수 있습니다.

**SNS 로그인(네이버/카카오/구글)도 로그인 후 휴대폰 등록·인증을 요구합니다.** 인증이 안 된
계정으로 로그인하면 번호 등록 게이트(`verify-phone`)로 이동하고, 인증을 완료해야 앱을 쓸 수
있습니다. (기존에 가입한 계정도 다음 로그인 시 한 번 인증이 필요합니다. 문자 발송사 미설정
시에는 화면의 테스트 코드로 인증하면 됩니다.) 게이트에서는 언제든 로그아웃할 수 있습니다.
