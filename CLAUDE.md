...
- **캔들차트(공용)**: `src/components/CandleChart.tsx` — 프로젝트 매매차트(`app/(app)/project/[id]/chart.tsx`)와 가치분석 화면이 같이 쓴다. 일/주/월봉·이평선 5·20·60·120·핀치 확대(짚은 곳 기준)·처음 3년치(왼쪽 끝까지 밀면 5년→10년)·가로 보기(화면 안 90° 회전 — 왼쪽 위에 종목명, 🔍로 다른 종목을 검색해 그 자리에서 둘러보기)·꾹 누른 채 움직이면 날짜·종가 크로스헤어 추적·그리기(선/가로선/세로선/화살표/사각형/원 + 꾹 눌러 선택해 이동·끝점으로 모양 수정, 종목·봉 종류별 AsyncStorage `chartDrawings:<symbol>:<mode>`). 체결가 가격선·매매 마커는 `lines`/`markers` props 로 프로젝트 차트에서만 넘긴다. 도형은 시간·가격으로 저장돼 확대·봉 전환에도 자리를 지킨다. 미국 키 없으면 안내 카드로 방어(앱 정상). 라우트는 `(app)/_layout.tsx`에 등록됨(뒤로가기).
...
- **체결 시차 vs 진짜 불일치**: 주문이 체결됐는데 앱이 아직 못 잡은 구간에서 '계좌와 수량이 다르다'는 가짜 경고가 떴다. `findHoldingMismatches`가 살아 있는 주문(status='sent')의 미체결 잔량을 함께 읽어, 차이가 `recorded − 매도잔량 ≤ held ≤ recorded + 매수잔량` 범위 안이면 `explainedByOrders=true`로 표시한다. 카드는 이 경우 경고 대신 '🕐 체결 확인 중'(회색)으로 낮춰 보여주고, 띄우기 전에 `reconcilePendingOrders`를 한 번 돌려 스스로 해소해 본다. 시간 기반 무음 처리가 아니라 '주문 잔량이 설명하는 만큼만' 봐주므로 진짜 불일치는 그대로 경고된다.
- **앱이 모르는 보유 종목**: 계좌에는 있는데 진행중 프로젝트가 없는 종목도 같은 카드에서 알린다(`findHoldingMismatches`가 국내·해외 잔고를 모두 읽어 `unmanaged: true`로 표시). '🩹 바로잡기' 한 번으로 `adoptHolding`(`src/services/adoptHolding.ts`)이 프로젝트 생성 + 포켓1 보유중(비중 100/0/0/0/0) + 평단 매수 체결 기록까지 한 번에 처리한다. **프로젝트 생성 화면을 거치지 않는 이유**: 그 화면은 예수금 기준 '사용가능 예산'을 넘으면 저장을 막는데, 이미 산 주식은 그 돈이 예수금에 없어 언제나 초과로 잡혀 만들 수가 없다. 기본 전략은 매수 간격 5%·매도 목표 10%(포켓별 '🎯 목표가 수정'으로 변경). 장기보유처럼 관리 대상이 아니면 '숨기기'(AsyncStorage `ignoredHoldings`)로 다시 알리지 않는다.
- **보유수량 불일치 바로잡기**: 앱 기록 ↔ 증권사 계좌가 어긋나면 프로젝트탭·포켓탭·매매일지 세 화면 모두 위쪽에 경고 카드(`src/components/HoldingMismatchCard.tsx`, 계좌 잔고를 스스로 조회·60초 캐시)와 '🩹 바로잡기' 버튼이 뜬다. 차이 수량만큼을 고른 포켓에 매수/매도 체결로 기록해 계좌와 맞춘다(`src/components/FixHoldingModal.tsx`). 매매일지의 '＋ 수동 입력'은 project_id/pocket_id 가 null 인 독립 기록이라 이 불일치를 못 고친다 — 안내 문구도 그렇게 적어 뒀다.
- **미체결 주문 취소·주문가 변경**: 매수·매도 모두 포켓 카드에 '🚫 …주문 취소'와 '✏️ 주문 취소하고 …가 변경'. 주문가 변경은 기존 주문을 취소한 뒤 새 가격으로 재주문 — 매수는 `buyPocket`, 매도는 `stopLossPocket(k, customPrice)`(현재가 대신 지정가). 주문가 변경 모달은 닫기/재주문 2개(취소는 카드에 있으므로 중복 제거).
- **표 최신화**: 전체 요약 표(`PortfolioSummary`) 제목 옆 '🔄' — 현재가를 다시 읽어 평가금액·평가이익을 즉시 갱신. 프로젝트탭은 `load()`, 포켓탭은 `load()`+`loadPrices()`. 자동 폴링은 `priceTracker` 30초(POLL_MS).
- **포켓 배분 3가지**(`src/lib/allocMode.ts` `useAllocMode`, 생성·수정 화면 공용): 비중 % / 금액 / **수량(주)**. 금액·수량 모드에서는 총예산 = 포켓 금액 합(총예산 칸 읽기 전용). 수량 모드는 포켓 금액 = 수량 × 그 포켓의 매수가(`priceOf` — 정액매수법은 `pocketBuyTarget`(기준가−간격%·i, KRX 호가 내림), 정기매수법은 현재가) 로 예산이 따라오고, 기준가·간격·현재가가 바뀌면 같은 수량으로 금액을 다시 계산한다. 비중은 소수 2자리로 정규화돼 '3주 × 71,000원'이 몇 원 모자라 1주 깎이던 문제가 있어, 금액·수량 모드는 `StrategyInput.budgets`로 포켓 배분액을 seeds 에 그대로 넘긴다(`buildPocketSeeds`/`buildScheduleSeeds` 둘 다 지원). 균등 분배·전액 입력·포켓 개수 변경은 수량 모드에서 `equalQtys`(총액÷개수÷매수가 내림).
- **매수법 두 가지**: `projects.buy_mode` — `'price'` = **정액매수법**(기준가에서 간격 %씩 내려갈 때마다 정해 둔 금액만큼 매수, 기존 방식) / `'schedule'` = **정기매수법**(정해진 날짜·시각에 현재가로 매수). 정기매수법은 `pockets.buy_at`(마이그레이션 20260909a)을 쓴다. 프로젝트 생성 화면에서 '📅 정기매수법'을 고르면 첫 매수 날짜·시각 + 간격(N일/주/개월)으로 포켓마다 예정 시각을 만든다(포켓 1개 = 매수 1회, `buildScheduleSeeds`). 생성 화면의 종목 카드는 **현재가를 표시만** 하고(🔄 재조회), **기준가 입력은 정액매수법을 골랐을 때만** 전략 카드에 나타난다(기본값 = 고른 순간의 현재가, '↺ 현재가로 맞추기'). 정기매수법은 기준가가 없으므로 현재가를 참고값으로 쓴다. 프로젝트 목록·상세는 정기매수법이면 기준가·매수간격 대신 **'9/12 ~ 11/14' + '📅 1주마다' + '+10%'**(`describeSchedule` — 주기는 포켓 간 간격에서 되짚어 냄)를 보여준다. **buy_at 이 있는 포켓은 가격 조건을 보지 않는다** — 예정 시각이 지나면 그때 **현재가 지정가**로 배분 예산이 허용하는 최대 수량을 산다(`evaluateSignals`가 `targetPrice=현재가`로 신호를 내므로 기존 `min(목표가,현재가)` 로직이 그대로 현재가가 된다). 매도는 기존과 동일(체결가 대비 `sell_target_pct`). 카드에는 목표가 대신 '📅 매수 예정 · 9/12 09:30 · D-3'(`formatBuyAt`)이 뜬다. 장이 닫혀 있으면 주문하지 않고 열릴 때까지 기다린다. 클라이언트 `autoTrader`와 서버 러너 양쪽에 동일 규칙. **정기매수법이 닿는 나머지 화면**: 프로젝트 수정(`edit.tsx`)은 기준가·매수간격 대신 첫 매수 날짜·시각·간격을 `inferSchedule`로 되짚어 보여주고 저장 시 `buildScheduleSeeds`로 `buy_at`까지 다시 쓴다(안 쓰면 날짜 수정이 반영되지 않음). 🎯 목표가 수정 모달은 정기매수법 대기 포켓이면 매수 목표가 대신 예정 날짜·시각을 고친다(`savePocketTargets(..., buyAt)`). 포켓 재시작은 예정 시각을 '지금 + 주기'로 옮긴다(그대로 두면 다음 폴링에 바로 사 버림). 프로젝트 복사는 `mode=schedule`을 넘겨 같은 방식으로 연다. 노란불·깜박임은 `buyPointReached`(정액=가격, 정기=시각)로 판정.
- **프로젝트 마지노선**(`projects.stop_price`, 마이그레이션 20260910a, `projectStopOf`/`belowStopLine` in src/domain/pockets.ts): 포켓 마지노선이 '보유 포켓 하나'의 손절선이라면 이건 '이 종목에서 손 뗄 선'. 현재가 ≤ 선이면 ① 대기중 포켓(정기·정액 모두)은 예정 시각·목표가에 닿아도 **매수 보류**(신호 없음, 노란불 꺼짐, 카드에 '🛑 마지노선 아래 · 매수 보류'), 선 위로 돌아오면 그대로 재개(예정 시각 지난 포켓은 바로 삼) ② 보유중 포켓은 포켓 마지노선이 없으면 이 값으로 `stop` 신호(현재가 전량 매도). `evaluateSignals(pockets, price, now, projectStop)`·`buyPointReached(..., projectStop)` 4번째 인자. 정기매수법 생성·수정 화면에 `StopLineField`(설명 포함), 수정 화면은 거래가 있어 잠겨도 별도 카드에서 '마지노선만 저장' 가능. 상세 헤더에 🛑 행(정기매수법은 항상, 정액은 설정 시). 러너는 stop_price 컬럼이 없으면 그 컬럼 없이 재조회(폴백)해 멈추지 않는다. **🎯 목표가 수정 모달의 마지노선 칸은 모든 포켓에 뜨고** '이 프로젝트 전체에 적용' 체크(보유중은 선택, 대기중은 항상 전체) → `applyStopToProject`(src/services/pocketTargets.ts)가 `projects.stop_price` 한 곳에 쓰고 포켓별 stop_price 는 모두 비운다(값을 포켓마다 복사하면 나중에 프로젝트 선을 고쳐도 포켓 값이 우선돼 어긋남). 모달 초기값은 포켓 → 프로젝트 순 폴백. 프로젝트 컬럼이 없으면 포켓 전부에 같은 값을 넣고 안내.
