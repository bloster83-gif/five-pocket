...
- **보유수량 불일치 바로잡기**: 앱 기록 ↔ 증권사 계좌가 어긋나면 포켓탭 위 경고 카드에 '🩹 바로잡기' 버튼. 차이 수량만큼을 고른 포켓에 매수/매도 체결로 기록해 계좌와 맞춘다(`src/components/FixHoldingModal.tsx`). 매매일지의 '＋ 수동 입력'은 project_id/pocket_id 가 null 인 독립 기록이라 이 불일치를 못 고친다 — 안내 문구도 그렇게 적어 뒀다.
