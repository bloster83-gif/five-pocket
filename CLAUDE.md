...
- **앱이 모르는 보유 종목**: 계좌에는 있는데 진행중 프로젝트가 없는 종목도 같은 카드에서 알린다(`findHoldingMismatches`가 국내·해외 잔고를 모두 읽어 `unmanaged: true`로 표시). '＋ 프로젝트 만들기'를 누르면 종목·시장·기준가(계좌 평단)·예산(평단×수량)을 채운 채 프로젝트 생성 화면이 열리고, 만든 뒤에는 일반 '🩹 바로잡기'로 보유분을 포켓에 붙이면 된다. 장기보유처럼 관리 대상이 아니면 '숨기기'(AsyncStorage `ignoredHoldings`)로 다시 알리지 않는다.
