...
export default function ProjectDetailScreen() {
  ...
  const stopLossPocket = async (
  ...
  ): Promise<{ ok: boolean; msg?: string; word?: '익절' | '손절'; ordered?: boolean }> => {
    ...
    let live = customPrice && customPrice > 0 ? customPrice : price;
    if (live == null || live <= 0) {
      try {
    ...
    if (tier === 'auto' && account && !kisOrderBlocked(project.market)) {
      ...
      try {
        ...
        // 체결수량이 주문수량보다 적으면 '부분체결' — 그만큼만 기록하고 남은 수량은 계속 기다린다.
  ...
  // 미체결 주문 취소 (매수·매도 공통).
  //  · 매수 취소 → '대기중' (부분체결로 이미 산 게 있으면 '보유중' 유지)
  //  · 매도 취소 → '보유중' — 엉뚱한 가격에 걸려 영영 체결되지 않는 주문을 풀고
  //    현재가로 다시 익절/손절할 수 있게 한다.
  const cancelPocketOrder = (k: Pocket) => {
    ...
    confirmAction(
      `${word} 주문 취소`,
      `포켓 ${k.idx + 1}의 미체결 ${word} 주문(${formatPrice(Number(po.order_price), project.market)} · ${money(rest, 0)}주)을 취소할까요?` +
        (done > 0 ? `\n이미 체결된 ${money(done, 0)}주는 그대로 남아요.` : '') +
        `\n취소하면 ‘${backTo}’으로 돌아가요.` +
        (!isBuy && project.close_after_sell ? '\n예약해 둔 ‘매도 후 자동 종료’도 함께 해제돼요.' : ''),
      async () => {
        try {
          ...
          // 그 경우 포켓이 '주문완료'에 영영 갇히지 않도록 기록만 정리하는 길을 열어준다.
          return chooseAction(
            '주문 취소 실패',
  ...
  const buyPocket = async (k: Pocket, customPrice?: number): Promise<{ ok: boolean; msg?: string }> => {
    ...
    if (tier === 'auto' && account && !kisOrderBlocked(project.market)) {
      ...
      try {
        ...
        // 부분체결이면 체결된 만큼만 기록하고 주문은 계속 살려 둔다.
        if (anyFill) {
          await supabase.from('trades').insert({
            user_id: session.user.id,
            project_id: project.id,
            ...
            quantity: fillQty,
            executed_at: new Date().toISOString(),
            note: `자동주문(KIS ${r.orderNo || '-'})` + (filled ? '' : ` · 부분체결 ${fillQty}/${qty}주`),
          });
          if (ordRow?.id) await markOrderProgress(ordRow.id, fillQty, filled);
        }
        await supabase
  ...
  return (
    ...
    <AutoOrderModal
      ...
      market={mkt}
      pending={orderChange}
      side={orderChange?.side === 'sell' ? 'sell' : 'buy'}
      sellQty={
        autoOrderPocket && orderChange?.side === 'sell'
          ? Math.floor(computePnL(trades.filter((t) => t.pocket_id === autoOrderPocket.id), null).totalQtyOpen)
          : 0
      }
      defaultPrice={
        autoOrderPocket
          ? (() => {
              ...
              const isSell = orderChange?.side === 'sell';
              const base = orderChange ? Number(orderChange.order_price) : autoOrderPocket.buy_target_price;
              return mkt === 'KRX' ? alignToKrxTick(base, isSell ? 'sell' : 'buy') : base;
            })()
          : 0
      }
      ...
      onSubmit={async (customPrice) => {
        ...
        const isSell = chg?.side === 'sell';
        const r = isSell
          ? await stopLossPocket({ ...k, status: 'bought' }, customPrice)
          : await buyPocket({ ...k, status: 'waiting' }, customPrice);
        await load();
        if (r.ok)
          notify(
            chg ? '주문가 변경 완료' : '자동주문 전송',
            `포켓 ${k.idx + 1} · ${formatPrice(customPrice, mkt)} 지정가로 ${
              chg ? `다시 ${isSell ? '매도 ' : ''}주문했어요.` : '자동주문을 넣었어요.'
            }`
          );
...
function AutoOrderModal({
  ...
  const qty = isSell ? sellQty : estimatedShares(budget, aligned);
  return (
      ...
      <Pressable onPress={onClose} style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.55)', justifyContent: 'center', padding: spacing.lg }}>
        ...
        >
          <Text style={{ color: colors.text, fontWeight: '900', fontSize: 18 }}>
            {pending
              ? `✏️ 포켓 ${pocket.idx + 1} ${isSell ? '매도' : '매수'} 주문가 변경`
              : `🤖 포켓 ${pocket.idx + 1} 자동주문`}
          </Text>
          ...
          <View style={{ flexDirection: 'row', gap: spacing.sm, marginTop: 2 }}>
            ...
            >
              <Text style={{ color: '#fff', fontWeight: '800', textAlign: 'center' }}>
                {pending ? '취소 후 이 가격으로 재주문' : '🤖 자동주문 넣기'}
              </Text>
            </Pressable>
          </View>
        </Pressable>
        ...
        <>
          ...
          {pendingWord ? (
            ...
            >
              ...
              {pendingOrder && (() => {
                // 부분체결(15주 주문 → 5주 체결)이면 아직 안 된 수량만 '남은 수량'으로 보여준다
                const totalQty = Math.floor(Number(pendingOrder.quantity));
                const doneQty = Math.floor(Number(pendingOrder.filled_qty ?? 0));
                const restQty = Math.max(0, totalQty - doneQty);
                return (
                <View style={{ marginTop: 6, gap: 3 }}>
                  ...
                  {doneQty > 0 && (
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                      <Text style={{ color: colors.textDim, fontSize: 12 }}>부분체결</Text>
                      <Text style={{ color: colors.warn, fontSize: 13, fontWeight: '800' }}>
                        {money(totalQty, 0)}주 중 {money(doneQty, 0)}주 체결
                      </Text>
                    </View>
                  )}
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              ...
              <Text style={{ color: colors.textDim, fontSize: 11, marginTop: 4 }}>
                전량 체결되면 자동으로 {pendingWord === '매수' ? '보유중' : '매도완료'}으로 바뀌어요.{'\n'}
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.md, marginTop: 6, flexWrap: 'wrap' }}>
                {pendingOrder && !projectClosed && (
                  <Pressable onPress={onChangeOrderPrice} hitSlop={6}>
                    <Text style={{ color: colors.primary, fontSize: 12, fontWeight: '800' }}>
                      ✏️ 주문 취소하고 {pendingWord}가 변경
                    </Text>
                  </Pressable>
                )}
                {/* 미체결 주문 취소 — 매수는 '대기중', 매도는 '보유중'으로 돌아간다 */}
                {pendingOrder && !projectClosed && (
                  <Pressable onPress={onCancelOrder} hitSlop={6}>
