                ...
                {maxIdx >= 0 && (
                  <View style={{ flexDirection: 'row', gap: 4 }}>
                    {Array.from({ length: maxIdx + 1 }, (_, i) => i).map((idx) => {
                      ...
                      const st = pk.status;
                      // 정기매수법은 가격이 아니라 '예정 시각이 지났는가'로 켜진다 (프로젝트 마지노선 아래면 꺼짐)
                      const reached = buyPointReached(pk, m?.price, Date.now(), projectStopOf(p));
                      const fill =
                        st === 'bought'
                          ? colors.buy
                          : st === 'buy_ordered'
                          ...
                          return (
                            <View style={{ width: 5 * dot + 4 * gap, flexDirection: 'row', flexWrap: 'wrap', gap, flexShrink: 0 }}>
                              {Array.from({ length: maxIdx + 1 }, (_, i) => i).map((idx) => {
                                ...
                                const st = pk.status;
                                // 정기매수법은 가격이 아니라 '예정 시각이 지났는가'로 켜진다 (프로젝트 마지노선 아래면 꺼짐)
                                const reached = buyPointReached(pk, m?.price, Date.now(), projectStopOf(item));
